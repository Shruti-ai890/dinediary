...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $category_name = $category_result->fetch_assoc()['category_name'] ?? 'Unknown Category';
    $stmt->close();

    // Fetch posts from admin_posts and blog_posts based on the category name
    $query = "(
    SELECT 
        ap.post_id AS id, 
        ap.title, 
...
<body>
    <header>
        ...
        <h1>Posts for Category: <?php echo htmlspecialchars($category_name, ENT_QUOTES, 'UTF-8'); ?></h1>
    </header>
