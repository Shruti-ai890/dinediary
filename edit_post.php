...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    $update_sql = "UPDATE blog_posts SET place_title = ?, description = ?, city = ?, area = ?, google_maps_link = ?, cuisine_name = ?, explore_name = ?, category_name = ?, updated_at = NOW() WHERE blog_id = ?";
    $update_stmt = $conn->prepare($update_sql);
    $update_stmt->bind_param("ssssssssi", $place_title, $description, $city, $area, $google_maps_link, $cuisine_name, $explore_name, $category_name, $blog_id);

    if ($update_stmt->execute()) {
        // Handle image upload if new images are provided
        if (!empty($_FILES['new_images']['name'][0])) {
            // Delete old images
            $delete_images_sql = "DELETE FROM blog_post_images WHERE blog_id = ?";
            ...
            foreach ($_FILES['new_images']['tmp_name'] as $key => $tmp_name) {
                ...
                $insert_image_sql = "INSERT INTO blog_post_images (blog_id, image_path) VALUES (?, ?)";
                $insert_image_stmt = $conn->prepare($insert_image_sql);
                $insert_image_stmt->bind_param("is", $blog_id, $image_name); // Save only the image name
                $insert_image_stmt->execute();
            }
        }

        // Redirect with success message
        header("Location: edit_post.php?blog_id=$blog_id&success=1");
        exit;
    } else {
...
<div id="edit-post-container">
    <h2 style="font-size: 2rem; text-align: center;">Edit Post</h2>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <label for="place_title">Place Title:</label>
        <input type="text" id="place_title" name="place_title" value="<?php echo $post['place_title']; ?>" required>
    ...
    <button type="submit" name="update" style="background-color: #DC143C; color: white; padding: 10px 20px; border-radius: 5px; cursor: pointer; transition: background-color 0.3s ease;" onmouseover="this.style.backgroundColor='#9B0F26'" onmouseout="this.style.backgroundColor='#DC143C'">Update Post</button>

    <a href="post_detail.php?blog_id=<?php echo $blog_id; ?>" style="background-color: #DC143C; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; cursor: pointer; transition: background-color 0.3s ease;" onmouseover="this.style.backgroundColor='#9B0F26'" onmouseout="this.style.backgroundColor='#DC143C'">Back</a>
</div>
